...
export const PlaylistForm = ({
	...
	onSubmit: ({ title, imageId }: { title: string; imageId?: Id<"files"> }) => Promise<void>;
	alertTitle?: string;
	alertMessage?: string;
	formTitle?: string;
	submitText?: string;
	dismissTo?: Href;
	playlistData?: undefined | { title: string; image?: { fileId: Id<"files">; url: string } };
}) => {
	const [isSubmitting, setIsSubmitting] = useState(false);
	const [title, setTitle] = useState(playlistData?.title ?? "");
	const [image, setImage] = useState<{ fileId: Id<"files">; url: string } | null>(playlistData?.image ?? null);
	const [uploading, setUploading] = useState(false);
	const { fetchAccessToken } = useCustomAuth();

	const isDirty = useMemo(() => {
		return (
			(title.trim().length > 0 && title !== playlistData?.title) ||
			(image !== null && image?.fileId !== playlistData?.image?.fileId)
		);
	}, [title, image, playlistData?.title, playlistData?.image?.fileId]);
	...
	const submitDisabled = useMemo(() => {
		return (
			...
			uploading ||
			isSubmitting ||
			(playlistData?.title === title && playlistData?.image?.fileId === image?.fileId)
		);
	}, [image?.fileId, isSubmitting, playlistData?.image?.fileId, playlistData?.title, title, uploading]);
	...
	const pickImage = useCallback(async () => {
		try {
			...
			setImage({ ...imageToUse, url: imageUrl, fileId });
			setUploading(false);
		} catch (e) {
			console.warn("[@/components/playlists/playlist-form.tsx]: Error picking image: ", e);
			await handleImageUploadError({
				text1: "Error uploading image",
				text2: "We hit a snag trying to uploade this image. Please try again later",
			});
	...
	const handleSubmit = useCallback(async () => {
		...
		await onSubmit({ title, imageId: image?.fileId });
		setIsSubmitting(false);
	}, [isSubmitting, title, onSubmit, image?.fileId]);

	usePreventFormDismiss({ isDirty, alertTitle, alertMessage });
