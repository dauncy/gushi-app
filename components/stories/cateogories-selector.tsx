...
export const CategoriesSelector = memo(
	({ onCategoryPress }: { onCategoryPress: (categoryId: string | null) => void }) => {
				...
				{isLoading ? (
					...
					<>
						{sanitizedCatgeories.map((category) => (
							<CategoryPill key={category.id} categoryData={category} onCategoryPress={onCategoryPress} />
						))}
					</>
				)}
			</View>
...
const CategoryPill = memo(
	({
		...
		const handleSelect = useCallback(async () => {
			...
			onCategoryPress(categoryData.id as Id<"categories">);
			updateCategoryId(categoryData.id as Id<"categories">);
			await Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Soft);
		}, [categoryData.id, onCategoryPress, selected]);
